...
app.post("/inputdb", upload.array("SkillImage"), function (req, res) {
  ...
  connection.query(sql1, param1, function (err, rows) {
    ...
    if (rows[0] !== undefined) {
      console.log("중복됨");
      res.send(
        "<script>alert('스킬명 중복됨');window.location.replace('/admin');</script>;"
      );
    } else {
      var sql =
        "Insert INTO skill (JobName,SkillName,SkillEx,AttHit,DefHit,SkillImg,SkillThumb) VALUES(?,?,?,?,?,?,?) ";
...
app.post("/inputpost", function (req, res) {
  ...
  let sql =
    "insert into post (id,password,Contents,Me,You,SkillName1,SkillName2,SkillName3) values (?,?,?,?,?,?,?,?)";
  let param = [
    req.body.id,
    req.body.password,
    req.body.contents,
